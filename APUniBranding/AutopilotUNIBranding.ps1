...
#Log function updated KH for date formatting and content
function Log() {
	[CmdletBinding()]
	param (
...
# *****Custom Code section sets specfic paths
if (-not (Test-Path "$($env:ProgramData)\CVMMPA")) {
	Mkdir "$($env:ProgramData)\CVMMPA" -Force
}

# Start logging
Start-Transcript "$($env:ProgramData)\CVMMPA\AutopilotUNIBranding.log"

# Creating tag file
Set-Content -Path "$($env:ProgramData)\CVMMPA\AutopilotUNIBranding.tag" -Value "Start Install $(get-date -f ""yyyy/MM/dd hh:mm:ss tt"")"
...
[xml]$GrpTag = Get-Content "$($Env:ProgramData)\CVMMPA\GrpTag.xml" 
...
<# ### comment STEP 6 out as all is being done in debloat already
...
Add-Content -Path "$($env:ProgramData)\CVMMPA\AutopilotUNIBranding.tag" -Value "Complete Install $(get-date -f ""yyyy/MM/dd hh:mm:ss tt"")"
Stop-Transcript
