...
Begin {
  ...
  $#This uses a "tag" file to determine whether the script has been run previously
	...
	$PSSTag = "$LogFolder\PlatformSequence.tag"


	Start-Transcript -Path "$LogFolder\Platform-Sequence.log"
	Log ""
	Log "******************************************************************************"
...
#	if ($DeBloat) {
		...
		Log "Calling Debloat and wait for completion"
...
#	if ($AutoBrand) {
		...
		if ($Verbose) {
			...
			start-process -FilePath "$PSHOME\powershell.exe" -Wait -WorkingDirectory "$APUniBrndFldr" -WindowStyle Hidden
